 ...
 * 7. Output to public/bangs.json and public/bangs.json.sha256 (then run split-bangs.js to finalize)
 */
...
const OUTPUT_SHA256_FILE = path.join(PUBLIC_DIR, 'bangs.json.sha256');
...
function normalizeUrl(url) {
  ...
  return url
    ...
    .replace(
      /^http:\/\/(www\.)?(google|youtube|wikipedia|github|reddit|amazon|twitter|duckduckgo)\./i,
      'https://$1$2.'
    )
    .trim();
}
...
async function main() {
  ...
  console.log(
    `   File size: ${(fileSize / 1024 / 1024).toFixed(2)} MB (${savings}% smaller than object format)`
  );
  ...
  console.log(
    `   Checksum file: ${OUTPUT_SHA256_FILE} (verify from repo root: sha256sum -c public/bangs.json.sha256)`
  );
